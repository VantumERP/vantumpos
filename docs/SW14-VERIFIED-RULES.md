...
| **W-16** | **To a labour lawyer, raised 08.08.2026 by the čl. 87 weekly leg going in:** *„Da li evidencija radnog vremena sme da odbije da upiše dan koji je maloletnik stvarno odradio, zato što je ta radna nedelja već preko 35 časova — ili je dužna da ga upiše, a prekoračenje da evidentira?“* Put the same question for the čl. 87 **daily** 8 h leg, which this module also implements as a refusal | **§4 req. 12 uses two different verbs in one sentence** — *„**reject** any overtime or preraspodela entry **outright** (čl. 88 st. 1), **cap** 35 h/week and 8 h/day (čl. 87)“* — and the module implements both as a hard refusal. The distinction matters because the neighbouring čl. 53 ceilings in the same module take the opposite route: the day **is** recorded and a čl. 53 st. 1 ground is demanded (*„Dan se može evidentirati, ali morate izabrati razlog prekoračenja“*). čl. 88 st. 1 prohibits *ordering* the work; čl. 87 caps its *quantity*; **neither says what the record must do once the hours have already been worked.** No primary text read in any pass addresses the register's duty in that state | **MEDIUM-HIGH, and reachable today.** As shipped, a minor's week already over 35 h admits no further worked day at **any** value — one minute is refused by the same arithmetic as eight hours — and `correct_entry` cannot reach a day with no stored version. The operator's remaining routes are to omit the day (ZoR čl. 55 st. 6 / ZEOR čl. 24 incompleteness, penalised by čl. 276 st. 1 tač. 1a) or to correct **other** days downwards, i.e. to record fewer hours than were worked. Both are false records — the exposure this module exists to remove — so the current behaviour does **not** fail safe. Reachable by a `datum_rodjenja` filled in late, by a restored backup, and by any row written before 07.08.2026. **Do not resolve it by choosing:** the permissive reading has the app help an employer past a minor-protection ceiling, and the strict reading is what ships. Pinned by `commands::worktime::tests::an_over_cap_minors_week_admits_no_further_worked_day`; a čl. 274 disclosure decision travels with whichever answer comes back |
