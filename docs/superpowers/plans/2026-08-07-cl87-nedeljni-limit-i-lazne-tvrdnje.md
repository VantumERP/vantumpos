...
1. **The archive claim is judged on the stems `arhiv` + `odobren` inside one paragraph, not on the literal `odobrenja arhiva`.** The statute's own wording is „bez pismenog odobrenja **nadležnog javnog** arhiva“, which contains neither of the two words adjacently and would have walked straight past the literal the plan named. The stem pair catches every case form and any interposed adjective. It deliberately does **not** bar the archive duties that are real (lista kategorija saglasnost, arhivska knjiga, 30 April prepis), and §4 item 8 — never tell a preduzetnik archive law does not reach him — is respected: nothing was added denying the duty, and the čuvanje sentence that remains is the čl. 9 st. 1 custody note in terms he can act on.
