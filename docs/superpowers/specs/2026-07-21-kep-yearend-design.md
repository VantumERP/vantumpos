...
- **Full-book paginated print** (`render_book_html`, §5.5): all the year's entries in the **fixed 5-column obrazac** (RB / Datum / Opis / Zaduženje 4 / Razduženje 5 — **no saldo column**, per KEP-VERIFIED-RULES §2.1), **grouped into pages of N rows** (e.g. 30), each page opening with a **DONOS** (cumulative carry-in) row and closing with a **SVEGA ZA PRENOS** (cumulative carry-out) row. Both carry the running **Σ kolona-4 and Σ kolona-5** totals *in their own columns* (per the §2.7 worked example: DONOS = opening / 0,00; SVEGA = cumulative col-4 / cumulative col-5). The **krajnji saldo is a derived line** under the last page (`Σ4 − Σ5`, čl. 16 st. 2), never a column. Numbered pages (`Strana {k} / {n}`); CSS `page-break-after` between page groups. The first page's DONOS kolona-4 = the year's opening carry-in (§2).
