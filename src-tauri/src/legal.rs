...
mod tests {
    ...
    /// only wrong under one of the two ZZP laws is still wrong, so listing
    /// `reklamacija_breach` once would leave the other regime unguarded.
    ...
    fn every_notice_function_is_enumerated_in_the_guard() {
        ...
        assert_eq!(
            ...
            "adding a notice function means adding it to all_notices, to the \
             ...
             regime, so `reklamacija_breach` contributes two"
        );
